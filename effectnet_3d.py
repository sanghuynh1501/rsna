...
from tensorflow.keras.layers import Conv3D, BatchNormalization, \
                                    Activation, Dropout, GlobalAveragePooling3D, \
                                    Reshape, Multiply, Add, GlobalMaxPooling3D, Dense
...
def mb_conv_block(inputs, block_args, activation, drop_rate=None, prefix='', ):
    ...
    if has_se:
        ...
        se_tensor = GlobalAveragePooling3D(name=prefix + 'se_squeeze')(x)

        target_shape = (1, 1, 1, filters)
        se_tensor = Reshape(target_shape, name=prefix + 'se_reshape')(se_tensor)
        se_tensor = Conv3D(num_reduced_filters, 1,
                                  activation=activation,
                                  padding='same',
                                  use_bias=True,
                                  kernel_initializer=CONV_KERNEL_INITIALIZER,
                                  name=prefix + 'se_reduce')(se_tensor)
        se_tensor = Conv3D(filters, 1,
                                  activation='sigmoid',
                                  padding='same',
                                  use_bias=True,
                                  kernel_initializer=CONV_KERNEL_INITIALIZER,
                                  name=prefix + 'se_expand')(se_tensor)
        x = Multiply(name=prefix + 'se_excite')([x, se_tensor])

    # Output phase
    x = Conv3D(block_args.output_filters, 1,
                      padding='same',
                      use_bias=False,
                      kernel_initializer=CONV_KERNEL_INITIALIZER,
...
def EfficientNet(width_coefficient,
    ...
    # Build stem
    x = img_input
    x = Conv3D(round_filters(32, width_coefficient, depth_divisor), 3,
                      strides=(2, 2, 2),
                      padding='same',
                      use_bias=False,
                      kernel_initializer=CONV_KERNEL_INITIALIZER,
    ...
    # Build top
    x = Conv3D(round_filters(1280, width_coefficient, depth_divisor), 1,
                      padding='same',
                      use_bias=False,
                      kernel_initializer=CONV_KERNEL_INITIALIZER,
                      name='top_conv')(x)
