...
import cv2
import numpy as np
from tf_data import AutoEncoderDataset, AutoEncoderImageDataset
...
batch_size = 32
train_data = AutoEncoderImageDataset(DATA_AUGMENT_TRAIN, X_train, batch_size).prefetch(tf.data.AUTOTUNE)
test_data = AutoEncoderImageDataset(DATA_AUGMENT_TRAIN, X_test, batch_size).prefetch(tf.data.AUTOTUNE)
